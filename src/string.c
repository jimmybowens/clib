...
/*int stringContains ( string *input, string *term)
{
   ...
   stringlist *words = splitString (input, ' ');
  int found = 0;
   stringlist *iterator = NULL;
   for( iterator = words; ! stringlistEnd ( iterator ); iterator = stringlistNext ( iterator ))
   {
     if( stringEqual ( iterator->data, term ))
     {
	found = 1;
	iterator = stringlistBreak ( iterator );
     }
   }
   words = stringlistDestroy ( words );
   return found;
}*/
...
int findIndexOfString(string *input, string *term, char delim, int count )
{
  ...
  while( *ptr != '\0')
  {
    ...
    idx = ( skips + noskips );
    cut = cutString ( input, start, end );
    if( stringEqual ( cut, term))
    {
      if( match_count >= count )
      {
	cut = deleteString ( cut );
	return start;
      }else
      {
	 index = start;
  ...
  {
     return ( index == 0 ? 0 : ( index - 1 ));//actual array index
  }
}
...
/*int findIndexOfString(string *input, string *term, char delim, int count )
{
  ...
  string *termDelim = makeString ( &delim );
  string *termStrippedFront = stripStringFront ( term, termDelim );
  string *termStrippedBack = stripStringBack ( termStrippedFront, termDelim );
  string *termDelin = delinString ( termStrippedBack,delim);

  stringlist *words = splitString ( input, delim);
  stringlist *iterator = NULL;
  for(iterator = words; ! stringlistEnd ( iterator ); iterator = stringlistNext ( iterator ))
  {
    len = getStringLength ( iterator->data);
    if( stringEqual ( iterator->data, termDelin))
    {
      printf ("%s %s\n",termDelin->value, iterator->data->value);
      idx += len;
      index = idx;
      //index = (idx + spaces - len);
      match++;
    }else
    {
      idx += len;
  ...
  words = stringlistDestroy ( words );
  termDelim = deleteString ( termDelim );
  termStrippedFront = deleteString ( termStrippedFront );
  termStrippedBack = deleteString ( termStrippedBack );
  termDelin = deleteString ( termDelin );
  return index;
}*/
int stringEqual(string *one, string *two)
{
  if( ! isValidString(one) || ! isValidString( two ))
  {
    return 0;
  }
  if( getStringLength(one) != getStringLength(two))
  {
    return 0;
  }
  char *ptr1 = &one->value[0];
  char *ptr2 = &two->value[0];
  while( *ptr1 != '\0')
  {
    if( *ptr1++ != *ptr2++)
    {
...
/*stringlist * addStringNext(stringlist *array, string *data)
  {
  if( array == NULL )
  {
  return NULL;
...
stringlist * splitString(string *strInput, char delim)
{
  ...
  if( wordCount == 1 )
  {
    head = stringlistAddFront ( head, copyString ( strInput));
    return head;
  }
  int cutStart = 0;
  int cutEnd = 0;
  ...
  while( *ptr != '\0')
  {
    if( *ptr == delim )
    {
      cutStart = last;
      cutEnd = idx;
      head = stringlistAddFront(head, ( cutString ( strDelin, cutStart, cutEnd)));
      last = (idx + 2);
    }
    idx++;
    ptr++;
  }
  head = stringlistAddFront(head, cutString(strDelin, findIndexOfChar(strDelin, delim, -1) + 2, -1) );
...
stringlist * splitString2(string *strInput, string *strDelim)
{
  ...
  stringlist * head = splitString ( source, ' ');
